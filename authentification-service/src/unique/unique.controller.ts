...
export class UniqueController {
    ...
    async isEmailUnique(@Payload() uniqueDto: UniqueDto): Promise<IResult> {
        return { data: await (await this.userService.findByEmail(uniqueDto.message)).email, error: {message: "OK", statusCode: HttpStatus.OK} }
    }
    ...
    async isUsernameUnique(@Payload() uniqueDto: UniqueDto): Promise<IResult> {
        try {
            return { data: await (await this.userService.findByUsername(uniqueDto.message)).username, error: {message: "OK", statusCode: HttpStatus.OK}  }
        }
        catch {
