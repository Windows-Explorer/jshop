<template>
    <q-form @submit="onSubmit()" @reset="onReset()" class="form">
        <logo class="logo"/>
        <q-input v-model="user.email" label="Электронная почта" :standout="quasar.dark.mode"
            :filled="!quasar.dark.mode" :type="'email'" :rules="validationRules.email"
            no-error-icon />
        <q-input v-model="user.password" label="Пароль" :standout="quasar.dark.mode"
            :filled="!quasar.dark.mode" :type="'password'" :rules="validationRules.password"
            :maxlength="16" :counter="true" no-error-icon />
        <span class="buttons">
